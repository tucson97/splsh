...
## 2. Setup en Shopify (10 min)

**La app se crea en el Dev Dashboard** (dev.shopify.com — es el flujo actual;
ya no se crean custom apps desde el admin de la tienda):

1. En tu app del Dev Dashboard → **Versiones / API access**: pedí los scopes
   `read_products`, `write_inventory`, `read_locations`, `write_files`,
   `read_orders` y publicá la versión.
2. **Distribución**: elegí distribución custom (una sola tienda), seleccioná
   tu tienda e instalala.
3. **Configuración → Credenciales**: copiá el **ID de cliente** y el
   **Secreto**. Esos son `SHOPIFY_CLIENT_ID` y `SHOPIFY_CLIENT_SECRET`.
   No hay token visible: el server lo genera y renueva solo con esas
   credenciales (client credentials grant).
4. **Blanks:** en el admin de la tienda creá un producto "Blanks" (no
   publicado en Online Store) con una variante por modelo usando los SKUs
   `BLANK-*`, con "Track quantity" activado y el stock real.
5. **Producto Custom Case:** variantes por modelo con SKUs `CUSTOM-*`,
   sin track de inventario.
6. **Webhook:** Settings → Notifications → Webhooks → Create webhook:
   evento `Order creation`, formato JSON, URL
   `https://TU-BACKEND/webhooks/orders-create`. Copiá el signing secret.
